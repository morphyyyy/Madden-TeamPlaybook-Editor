    ...
    public class PBAU
    {
            ...
            for (int i = 0; i < TableProps.RecordCount; i++)
            {
                if (filter != 0 && (int)(UInt32)TDB.TDBFieldGetValueAsInteger(DBIndex, TDB.StrReverse("PBAU"), TDB.StrReverse("PBPL"), i) == filter)
                {
                    _PBAU.Add(new PBAU
                    {
                        ...
                        BOKL = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(DBIndex, TDB.StrReverse("PBAU"), TDB.StrReverse("BOKL"), i),
                        ...
                        Flag = (int)(UInt32)TDB.TDBFieldGetValueAsInteger(DBIndex, TDB.StrReverse("PBAU"), TDB.StrReverse("Flag"), i)
                    });
                }
                else if (filter == 0)
                {
        ...
        public static void SetPBAU(List<PBAU> PBAU, int DBIndex = 0)
        {
            ...
            foreach (PBAU item in PBAU)
            {
                ...
                TDB.TDBFieldSetValueAsInteger(DBIndex, TDB.StrReverse("PBAU"), TDB.StrReverse("BOKL"), item.rec, item.PBPL);
                ...
                TDB.TDBFieldSetValueAsInteger(DBIndex, TDB.StrReverse("PBAU"), TDB.StrReverse("Flag"), item.rec, item.pbau);
            }
        }
